...
void Pipeline<primitive_type, Program, flags>::run(std::vector<Vertex> const& vertices,
	...
	for (auto const& f : fragments) {
		...
		for (uint32_t s = 0; s < samples.size(); ++s) {
			...
			float sx = f.fb_position.x + samples[s].x;
			float sy = f.fb_position.y + samples[s].y;

			// fragment location (in pixels):
			int32_t x = (int32_t)std::floor(sx);
			int32_t y = (int32_t)std::floor(sy);
...
void Pipeline<p, P, flags>::rasterize_line(
	...
	auto inDiamond = [](float x, float y) {
		if (x - std::floor(x) + y - std::floor(y) == 0.5f) {
			return true;
		}
		return std::abs(x - std::floor(x) - 0.5f) + std::abs(y - std::floor(y) - 0.5f) < 0.5f;
	};

	// get the quadrant a point is in (-1 = in-diamond, 0 = bottom-left, 1 = bottom-right, 2 = top-left, 3 = top-right)
	auto getQuadrant = [&](float x, float y) {
		if (inDiamond(x, y)) {
			return -1;
		} else if ((x - std::floor(x)) + (y - std::floor(y)) < 0.5f) {
			return 0;
		} else if ((x - std::floor(x)) + (y - std::floor(y)) > 1.5f) {
			return 3;
		} else if ((x - std::floor(x)) - (y - std::floor(y)) > 0.5f) {
			return 1;
		} else {
	...
	// if starting in right bottom or left top, need to project a[0] to halfway point 
	// and check if a[1] is in the diamond
	if (inDiamond(a[0], a[1]) || getQuadrant(a[0], a[1]) == 0) {
		drawLine(floor(a[0]) + 0.5f, floor(a[1]) + 0.5f, a[2]);
	} else if (getQuadrant(a[0], a[1]) != 3) {
		float w = (floor(a[0]) + 0.5f - a[0]) / (b[0] - a[0]);
		float v = w * (b[1] - a[1]) + a[1];
		if (inDiamond(floor(a[0]) + 0.5f, v)) {
			drawLine(floor(a[0]) + 0.5f, floor(a[1]) + 0.5f, a[2]);
		}
	}

	if (inDiamond(b[0], b[1]) || getQuadrant(b[0], b[1]) == 3) {
		drawLine(floor(b[0]) + 0.5f, floor(b[1]) + 0.5f, b[2]);
	} else if (getQuadrant(b[0], b[1]) != 0) {
		float w = (floor(b[0]) + 0.5f - a[0]) / (b[0] - a[0]);
		float v = w * (b[1] - a[1]) + a[1];
		if (inDiamond(floor(b[0]) + 0.5f, v)) {
			drawLine(floor(b[0]) + 0.5f, floor(b[1]) + 0.5f, b[2]);
		}
	}
	...
	if constexpr ((flags & PipelineMask_Interp) == Pipeline_Interp_Flat) {
		...
		for (int i = x_min; i <= x_max; i++) {
			for (int j = y_min; j <= y_max; j++){
				...
				frag.fb_position = q;
				frag.attributes = va.attributes;
				frag.derivatives.fill(Vec2(0.0f, 0.0f)); // no derivatives for flat shading
				emit_fragment(frag);
			}
		}
		...
		for (int i = x_min; i <= x_max; i++) {
			for (int j = y_min; j <= y_max; j++){
				...
				float ux = fragx.attributes[0];
				float vx = fragx.attributes[1];
				float uy = fragy.attributes[0];
				float vy = fragy.attributes[1];
				...
				frag.derivatives[0] = Vec2(dudx, dudy);
				frag.derivatives[1] = Vec2(dvdx, dvdy);
		...
		for (int i = x_min; i <= x_max; i++) {
			for (int j = y_min; j <= y_max; j++){
				...
				for (int i = 0; i < va.attributes.size(); i++) {
					...
					float p_interp = fragx_weights[0] * p_a + fragx_weights[1] * p_b + fragx_weights[2] * p_c;
					float z_interp = fragx_weights[0] * va.inv_w + fragx_weights[1] * vb.inv_w + fragx_weights[2] * vc.inv_w;
					fragx.attributes[i] = p_interp / z_interp;
				}
				...
				for (int i = 0; i < va.attributes.size(); i++) {
					...
					float p_interp = fragy_weights[0] * p_a + fragy_weights[1] * p_b + fragy_weights[2] * p_c;
					float z_interp = fragy_weights[0] * va.inv_w + fragy_weights[1] * vb.inv_w + fragy_weights[2] * vc.inv_w;
					fragy.attributes[i] = p_interp / z_interp;
				}
